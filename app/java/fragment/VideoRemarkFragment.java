...
public class VideoRemarkFragment extends Fragment {
    ...
    String result;
    View lsdylist;
    String course_id;//接收传过来的id

    public VideoRemarkFragment(String course_id)
    {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState){
        view=inflater.inflate(R.layout.fragment_video_remark,null);
        return view;
    }
    @Override
    ...
    public void init()
    {
        GetVideoCommentsThread getGetVideoCommentsThread=new GetVideoCommentsThread();
        getGetVideoCommentsThread.start();
        try {
            getGetVideoCommentsThread.join();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }

        initListView();
        viewPager=view.findViewById(R.id.video_comments_Pager);//获取热门评论的ViewPager
        ...
        add.setOnClickListener
                (e-> {
                            ...
                            if (result.equals("1"))
                            {
                                Toast.makeText(getContext(),"评论发表成功",Toast.LENGTH_SHORT).show();
                            }
                            //收起键盘
                            fbtext.setText("");
    ...
    private class GetVideoCommentsThread extends Thread
    {
        @Override
        public void run() {
            result= InforUtil.getAllVideoComments(course_id);
